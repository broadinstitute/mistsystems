...
class Psk:
    ...
    def set_passphrase(self, value):        
        if len(value) >= 8 and len(value)<=63:            
            ...
            logging.error("Passphrase must be 8 to 63 characters long, or 64 HEX.")
            return False
    ...
    def set_ssid(self, value):
        if value != "":          
            ...
            logging.error("Value %s is not accepted. Please enter a valid SSID name." %value)
            return False
    ...
    def set_usage(self, value):
        if value in usages_list:
            ...
            logging.warning("Value %s is not accepted. Only \"multi\" and \"single\" are valid choices." % value)
        return True
        ...
        if value == "" or value == 0:
            ...
            try:
                ...
                if vid > 0 and vid < 4095:
                    ...
                    logging.error("Value %s is not accepted. Please enter a valid VLAN Id (1 to 4094)." % value)
                    return False
            except:
                logging.error("Value %s is not accepted. Please enter a valid VLAN Id (1 to 4094)." % value)
                return False  
        ...
        if value == "":
            ...
            try:
                ...
                if re.match('^[0-9A-F]*$', value):
                    ...
                    logging.error("%s is not a valid MAC Address format." % value)
            except:
                logging.error("%s is not a valid MAC Address format." % value)
                return False
